...
export function send(to: string, subject: string, variables: TemplateData, attachment?: mailgun.AttachmentParams): Promise<mailgun.messages.SendResponse> {
    let attach = undefined;

    if (attachment) {
        const buffer = new Buffer(attachment.data as string);
        attach = new mg.Attachment(Object.assign({}, attachment, {data: buffer, knownLength: buffer.length}));
    }
