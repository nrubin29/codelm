...
async function sendTeacherEmails() {
    for (const teacher of teachers) {
        ...
        console.log(teacher[3], teacher[2].split(' ')?.[0] ?? '', myStudents.length);

        console.log(await send(teacher[3], '[Please read] Important information for CodeLM 2020', {
            templateName: 'info-teachers',
            firstName: teacher[2].split(' ')?.[0] ?? '',
        }, {
            filename: 'accounts.csv',
            contentType: 'text/csv',
            data: 'name,email address,division,username,password\n' +
                myStudents.map(row => row.filter(((_, index) => [2, 3, 5, 6, 7].includes(index))).join(',')).join('\n'),
        }));
    }
}
