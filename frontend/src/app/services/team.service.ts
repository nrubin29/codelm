  ...
  constructor(private restService: RestService, private divisionService: DivisionService) {
    super({
      ...
      attributes: [
        ...
        {name: 'password'},
        {name: 'members'},
        {name: 'division', type: 'select', options: () => this.divisionService.getAll().then(divisions => divisions.map(division => ({name: division.name, value: division._id.toString()}))), transform: (value?: Partial<TeamModel>) => value?.division?._id},
      ]
    });
