...
import {PersonExperience, PersonModel, PersonYear} from '../../../../common/src/models/person.model';
...
export class PersonService extends GroupedEntityService<PersonModel, GroupModel> {
  private endpoint = 'people';

  constructor(private restService: RestService, private groupService: GroupService) {
    super({
      entityName: 'person',
      columns: [
      ...
      attributes: [
        ...
        {name: 'experience', type: 'select', options: Object.values(PersonExperience)},
        {name: 'group', type: 'select', options: () => this.groupService.getAll().then(groups => groups.map(group => ({name: group.name, value: group._id.toString()})))},
      ],
      editable: true,
    });
  }
  ...
  getChildren(parent: GroupModel): Promise<PersonModel[]> {
    return this.restService.get<PersonModel[]>(`${this.endpoint}/group/${parent._id}`);
  }
